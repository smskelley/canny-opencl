...
cl::Kernel OpenclImageProcessor::loadKernel(string filename, string kernel_name,
  ...
  try {
    ...
    cerr << "Build Status:\t"
         ...
         << endl << "Build Log:\t "
         << program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(selectedDevice) << endl;
  }
...
void OpenclImageProcessor::LoadImage(cv::Mat &image_input) {
  ...
  nextBuff() = cl::Buffer(
      ...
      input.rows * input.cols * input.elemSize(), input.data);
  prevBuff() = cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
                          input.rows * input.cols * input.elemSize());

  // Initialize the theta buffer
  theta = cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
                     input.rows * input.cols * input.elemSize());
  advanceBuff();
}
...
void OpenclImageProcessor::FinishJobs() { queue.finish(); }
...
void OpenclImageProcessor::Gaussian() {
  try {
    ...
    queue.enqueueNDRangeKernel(gaussian, cl::NDRange(1, 1),
                               cl::NDRange(input.rows - 2, input.cols - 2),
                               cl::NDRange(groupSize, groupSize), NULL);
...
void OpenclImageProcessor::Sobel() {
  ...
  queue.enqueueNDRangeKernel(sobel, cl::NDRange(1, 1),
                             cl::NDRange(input.rows - 2, input.cols - 2),
                             cl::NDRange(groupSize, groupSize), NULL);
...
void OpenclImageProcessor::NonMaxSuppression() {
  ...
  queue.enqueueNDRangeKernel(nonMaxSuppression, cl::NDRange(1, 1),
                             cl::NDRange(input.rows - 2, input.cols - 2),
                             cl::NDRange(groupSize, groupSize), NULL);
...
void OpenclImageProcessor::HysteresisThresholding() {
  ...
  queue.enqueueNDRangeKernel(hysteresisThresholding, cl::NDRange(1, 1),
                             cl::NDRange(input.rows - 2, input.cols - 2),
                             cl::NDRange(groupSize, groupSize), NULL);
